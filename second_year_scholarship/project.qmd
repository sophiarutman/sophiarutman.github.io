...
1. **Implement preventative measures based on the connections between organizations, attack context, motivation, and location.** I mentioned many connections above. By passing these on to a committee or specific person who knows more about the storage of important artifacts and are safest spots duirng crossfire, they can use these correlations to move items, advise aid workers on where to locate themselves, and increase or decrease security as necessary. In countries of high incidence rates like Afghanistan or South Sudan, it would be helpful for an expert on each respective country to formulate ideas based on these findings so that the policies implemented are most efficent for regions most dangerous to aid workers. 
2. **Improve reporting of gender and location.** These two categories are among the easiest to identify upon arriving on the scene. If these can be reported right away or recorded in some form for later report it would greatly strengthen the database, as well improve all organizations' ability to implement the recommendation above. In addition, with improvement of these two categories, predictive modelling may become possible. I specifically mention the importance of these two categories because of the ease of reporting these two as well as the correlations between the unknown categories of these variables. They have two of the highest correlations out of any of the unknown values across multiple other variables. By properly reporting these variables, location and gender may be influential factors in models created to categorize the "unknown" values of other variables. For example, if the motive of a certain incident is unknown, reporting location and gender may be the difference maker in whether many different factors can be combined within a machine learning algorithm to predict this motive. 